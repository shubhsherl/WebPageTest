  ...
  if (isset($cmd) && strlen($cmd)) {
    ...
    if ($flags && is_array($flags) && count($flags)) {
      foreach($flags as $flag) {
        if (strlen($flag) && !preg_match('/^--(([a-zA-Z0-9\-\.\+=,_< "]+)|((snappi-hints-103|snappi-resource-delay-urls|snappi-hint-root-url|snappi-hints|ignore-certificate-errors-spki-list|enable-features|optimization-guide-service-get-hints-url|ukm-server-url|metrics-upload-interval|prism-test-label|force-enable-metrics-reporting|data-reduction-proxy-http-proxies|data-reduction-proxy-config-url|proxy-server|proxy-pac-url|force-fieldtrials|force-fieldtrial-params|trusted-spdy-proxy|origin-to-force-quic-on|oauth2-refresh-token|unsafely-treat-insecure-origin-as-secure|user-data-dir)=[a-zA-Z0-9\-\.\+=,_:\/"%]+))$/', $flag)) {
